...
<br></br>
<p align="right"><input type="Button" value="PRINT" onclick="javascript:window.print()"></input></p>
<%
...
try
{
	...
	rs3=db3.executeDBQuery("select empid from employees");
	while(rs3.next())
	{
	rs1=db1.executeDBQuery("select * from "+table+" where empid='"+rs3.getString(1)+"' and month='"+month+"'");
	if(rs1.next())
	{
	rs2=db2.executeDBQuery("select cls,els from leaves where empid='"+rs3.getString(1)+"'");
	if(rs2.next())
	{
		cl=rs2.getString(1);
		el=rs2.getString(2);
